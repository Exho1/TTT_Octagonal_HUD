...
surface.CreateFont("WepSwitch_ex", {font = "Octin Sports RG", size = 18, weight = 500})
...
local col_dark = {
	tip = {
		[ROLE_INNOCENT] = Color(39, 194, 56, 160),
		[ROLE_TRAITOR] = Color(182, 37, 23, 160),
		[ROLE_DETECTIVE] = Color(41, 108, 175, 160),
	},

	bg = Color(40, 49, 58, 200),
...
function WSWITCH:DrawWeapon(x, y, c, wep)
	...
	-- Slot
	local spec = {text = wep.Slot + 1, font = "WepSwitch_ex", pos = {x + 0, y}, yalign = TEXT_ALIGN_CENTER, color = c.text}
	draw.TextShadow(spec, 0, c.shadow)

	-- Name
	spec.text = name
	spec.font = "TimeLeft_ex"
...
function WSWITCH:Draw(client)
	...
	for k, wep in pairs(weps) do
		...
		self:DrawBarBg(x - 30, y - 15, width, height, col)

		if not self:DrawWeapon(x - 30, y - 15, col, wep) then
